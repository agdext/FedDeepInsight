...
if __name__ == "__main__":
    ...
    dataset_path = '/Users/allan/dataset/stroke_test.csv'
    ...
    class IrisClient(fl.client.NumPyClient):
        ...
        def evaluate(self, parameters, config):  # type: ignore
            ...
            # Define file path
            csv_file_path = f"StrokeLR2025_metrics_per_round_{N_CLIENTS}C.csv"

            # Ensure headers exist: write them if file is missing or empty
            write_headers = not os.path.isfile(csv_file_path) or os.path.getsize(csv_file_path) == 0

            # Save to CSV
            with open(csv_file_path, mode="a", newline="") as csv_file:
                writer = csv.DictWriter(csv_file, fieldnames=["Round", "Loss", "Accuracy", "Precision", "Recall", "F1-Score"])
                if write_headers:
                    writer.writeheader()
